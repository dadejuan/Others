...
for(i in 1:size_positive_coloc)
{
	...
	#Get frequecy for the presence of both features in the pair
	size_freqs<-length(freqs)
	int_freqs[,eval(int_label):=empty_states]
	temp<-empty_states
	for(j in 1:size_freqs)
	{
		#temp[comb_counts[comb_counts[,positive_coloc$HM[i]]==1 & comb_counts[,positive_coloc$CM[i]]==1,]$State[j]]<-comb_counts[comb_counts[,positive_coloc$HM[i]]==1 & comb_counts[,positive_coloc$CM[i]]==1,]$freq[j]/(states_counts)$freq[comb_counts$State[j]]
